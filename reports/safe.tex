...
\subsection{Methodology: STL Decomposition}
To understand the underlying structure of the electricity demand, we employ Seasonal-Trend decomposition using Loess (STL). This method decomposes the time series $y_t$ into three additive components: a trend-cycle component $T_t$, a seasonal component $S_t$, and a remainder component $R_t$, such that $y_t = T_t + S_t + R_t$. STL is particularly robust to outliers and can handle any type of seasonality, making it ideal for high-frequency energy data \cite{Cleveland1990,Hyndman2021}.

\subsection{Component Analysis}
Figure~\ref{fig:stl_components} illustrates the decomposition of the hourly demand series. The trend component reveals the longer-term variations, potentially driven by seasonal weather changes or gradual shifts in household occupancy. The seasonal component captures the strictly repeating daily pattern (24-hour cycle), which is the dominant feature of household electricity usage. The remainder component contains the random fluctuations and noise that cannot be explained by the trend or the fixed seasonal pattern.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/demand_stl_components.png}
  \caption{STL Decomposition of hourly demand. The strong seasonal component confirms the diurnal nature of the data.}
  \label{fig:stl_components}
\end{figure}

\subsection{Seasonality Strength}
We quantify the strength of the seasonal patterns to justify the need for seasonal modeling approaches. As shown in Figure~\ref{fig:seasonality_strength}, the daily (24-hour) seasonality is significantly stronger than the weekly seasonality. This suggests that while there are differences between weekdays and weekends, the primary driver of variance is the time of day.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/demand_seasonality_strength.png}
  \caption{Strength of seasonality components. The daily cycle is the dominant temporal feature.}
  \label{fig:seasonality_strength}
\end{figure}

\subsection{Typical Profiles}
To further investigate the daily patterns, we analyze typical hourly profiles. Figure~\ref{fig:typical_profiles} compares the average demand profile for weekdays versus weekends. Weekdays typically exhibit distinct morning and evening peaks, corresponding to pre-work and post-work activities. Weekends, in contrast, show a flatter profile with higher midday demand, reflecting different occupancy patterns. These insights are crucial for feature engineering, as they highlight the interaction between the hour of the day and the day of the week.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/demand_typical_hourly_weekday_vs_weekend.png}
  \caption{Typical hourly demand profiles for weekdays vs. weekends. Note the distinct morning and evening peaks on weekdays.}
...
%===========================
\section{Statistical Models (ARIMA)}

\subsection{Stationarity and Differencing}
Statistical forecasting models like ARIMA require the time series to be stationary, meaning its statistical properties (mean, variance) do not change over time. We assess stationarity using the Augmented Dickey-Fuller (ADF) test and the KPSS test. Visual inspection of the Autocorrelation Function (ACF) and Partial Autocorrelation Function (ACF) plots (Figure~\ref{fig:acf_pacf}) reveals significant lags at 24-hour intervals, confirming the strong seasonality observed in the decomposition analysis. To achieve stationarity, we apply seasonal differencing (lag 24) and, if necessary, first-order differencing.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/stats_acf_pacf.png}
  \caption{ACF and PACF plots of the demand series. The spikes at multiples of 24 indicate strong daily seasonality.}
  \label{fig:acf_pacf}
\end{figure}

\subsection{Model Selection (SARIMA)}
We employ Seasonal ARIMA (SARIMA) models, denoted as $ARIMA(p,d,q)(P,D,Q)_m$, where $m=24$ for hourly data. We perform a grid search over the hyperparameters to minimize the Akaike Information Criterion (AIC). The best-performing model is typically a SARIMA model that includes seasonal autoregressive (SAR) and seasonal moving average (SMA) terms to capture the daily cycle.

\subsection{Model Evaluation}
The selected model is evaluated on a hold-out validation set. Figure~\ref{fig:stats_forecast} shows the forecast overlay for the best statistical model. The model successfully captures the daily peaks and troughs, although it may struggle with sharp spikes or irregular behavior driven by external factors not included in the univariate series.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/stats_forecast_overlay_best.png}
  \caption{Forecast overlay of the best SARIMA model against actual demand. The model captures the diurnal rhythm well.}
  \label{fig:stats_forecast}
\end{figure}

Performance metrics such as Root Mean Squared Error (RMSE) and Mean Absolute Error (MAE) are calculated to quantify the model's accuracy. These baselines serve as a benchmark for the more complex machine learning models.

%===========================
% 8. Machine Learning Models
%===========================
\section{Machine Learning Models (XGBoost)}

\subsection{Model Architecture}
To address the limitations of linear statistical models and capture non-linear relationships, we implement an XGBoost (Extreme Gradient Boosting) regressor. XGBoost is a powerful ensemble learning method based on decision trees, known for its high performance and efficiency in structured data problems \cite{Chen2016}.

\subsection{Feature Engineering for ML}
Unlike ARIMA, which relies on the internal structure of the target variable, XGBoost requires a feature matrix $X$. We construct a rich set of features including:
\begin{itemize}
    \item \textbf{Temporal Features:} Hour of day, day of week, is\_weekend, month.
    \item \textbf{Lag Features:} Demand at $t-1$, $t-24$, $t-168$ (previous week) to capture persistence and seasonality.
    \item \textbf{Rolling Statistics:} Rolling mean and standard deviation over windows of 3, 6, and 24 hours.
\end{itemize}

\subsection{Training and Validation}
The model is trained on the historical dataset, with the last 7 days held out for validation (Whole-train split). We use a robust set of hyperparameters (e.g., 600 estimators, learning rate 0.05) to prevent overfitting while ensuring sufficient model complexity. Figure~\ref{fig:ml_forecast} displays the XGBoost forecast on the validation set.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/08_forecast_overlay.png}
  \caption{XGBoost forecast overlay. The model demonstrates a strong ability to track the complex daily profile.}
  \label{fig:ml_forecast}
\end{figure}

\subsection{Performance Comparison}
The XGBoost model generally outperforms the statistical baselines in terms of RMSE and MAE, primarily due to its ability to learn complex interactions between time-of-day and recent demand history. The non-linear nature of the decision trees allows it to better adapt to the sharp changes in demand during morning and evening peaks.
...
\subsection{Rolling Forecast Methodology}
To simulate a realistic operational environment, we implement a rolling forecast pipeline. In this setup, the model predicts the next 24 hours, then the window rolls forward, and the process repeats for a total of 7 days. This approach provides a more rigorous evaluation of the model's stability and performance over time compared to a single static split.

\subsection{Model Comparison}
We compare the performance of the XGBoost model against the best statistical model (SARIMA) and simple baselines (Naive and Seasonal Naive). Figure~\ref{fig:pipeline_overlay} shows the overlay of these models for a representative day.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/09_day_overlay_rep.png}
  \caption{Comparison of forecasting models for a representative day. XGBoost and BestStat track the actual demand closely, while Naive baselines lag significantly.}
  \label{fig:pipeline_overlay}
\end{figure}

\subsection{Aggregate Results}
The aggregated metrics over the 7-day rolling window confirm the superiority of the XGBoost model. It consistently achieves the lowest Normalized RMSE (nRMSE), indicating that it provides the most reliable forecasts for the HEMS optimization task. The ability to incorporate multiple lag features and non-linear temporal interactions gives it a decisive edge over the univariate SARIMA model.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/09_week_overlay_best.png}
  \caption{Seven-day rolling forecast of the best performing model (XGBoost). The stability of the forecast over the week is evident.}
...
\subsection{Integrating External Drivers}
Household energy demand is not an isolated system; it is heavily influenced by external factors. We extend our models to include exogenous variables:
\begin{itemize}
    \item \textbf{Weather:} Ambient temperature affects heating and cooling loads.
    \item \textbf{Solar Generation (PV):} While PV is a supply-side variable, it correlates with time-of-day and weather conditions that also drive demand (e.g., cloud cover).
\end{itemize}

\subsection{ARIMAX vs. XGBoost with Exogenous Features}
We compare an ARIMAX model (ARIMA with eXogenous regressors) against the XGBoost model enriched with these new features. Figure~\ref{fig:exog_comparison} illustrates the performance of both models.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/10_exog_forecast_comparison.png}
  \caption{Forecast comparison with exogenous variables. Both models benefit from the additional information, but XGBoost handles the multi-variate inputs more effectively.}
  \label{fig:exog_comparison}
\end{figure}

\subsection{Feature Importance}
Analysis of feature importance (Figure~\ref{fig:exog_importance}) reveals that while autoregressive lags (past demand) remain the strongest predictors, temperature and hour-of-day play significant roles. This confirms that weather sensitivity is a measurable component of the household's energy profile.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/10_exog_feature_importance.png}
  \caption{Feature importance for the exogenous XGBoost model. Lags and temperature are key drivers.}
...
The final stage of the data science lifecycle is decision-making. We formulate a linear programming (LP) problem to optimize the operation of a Home Energy Management System (HEMS) equipped with a battery storage system. The objective is to minimize the total electricity cost over a 24-hour horizon.
\begin{equation}
    \min \sum_{t=1}^{24} (P_{grid}^{import}(t) \cdot C_{buy}(t) - P_{grid}^{export}(t) \cdot C_{sell}(t))
\end{equation}
Subject to constraints on:
\begin{itemize}
    \item \textbf{Energy Balance:} Demand must be met by Grid, PV, or Battery.
    \item \textbf{Battery Dynamics:} State of Charge (SOC) evolution with efficiency losses.
    \item \textbf{Physical Limits:} Battery capacity (10 kWh), inverter power limits (5 kW).
\end{itemize}

\subsection{Scenario Analysis}
We evaluate the optimization under two scenarios: Low PV generation (cloudy day) and High PV generation (sunny day). The optimization algorithm (implemented using \texttt{cvxpy}) determines the optimal charging and discharging schedule to arbitrage price differences and maximize self-consumption.

\subsection{Results and Economic Impact}
The optimization results demonstrate significant cost savings. By charging the battery during low-price periods or from excess PV, and discharging during high-price peak hours, the system reduces grid reliance. Table~\ref{tab:optim_summary} summarizes the economic and energetic performance.

\begin{table}[H]
  \centering
  \caption{Optimization Summary for Low and High PV Scenarios.}
  \label{tab:optim_summary}
  \csvautobooktabular{tables/11_storage_optimization_summary.csv}
\end{table}

The "High PV" scenario naturally results in lower costs (and potentially negative costs, i.e., profit) due to substantial energy exports. The battery cycles are utilized to shift this abundant energy to times of demand or high sell prices, maximizing the value of the solar investment.
